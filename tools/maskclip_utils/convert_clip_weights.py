    ...
    for key in state_dict.keys():
        if 'ViT' in args.model and prefix in key:
            ...
            if 'transformer' in key:
                new_key = 'layers.' + key[len('transformer.resblocks.'):]
                if 'mlp' in new_key:
                    new_key = new_key.replace('mlp', 'ffn.layers')
                if 'c_fc' in new_key:
                    new_key = new_key.replace('c_fc', '0.0')
                ...
                # 'positional_embedding', 'text_projection', 'logit_scale', 'token_embedding', 'ln_final'
                new_key = key
                if 'ln_final' in new_key:
                    new_key = new_key.replace('ln_final', 'ln0')
                if 'logit_scale' in new_key:
                    all_model['logit_scale'] = state_dict[key].float()
                    continue
            text_model['state_dict'].update({new_key: state_dict[key].float()})

    if args.backbone:
        torch.save(result_model, f'clip/{args.model}_clip_backbone.pth')
    else:
        all_model['visual'] = result_model['state_dict']
        all_model['text'] = text_model['state_dict']
        torch.save(all_model, 'clip/{}_clip_weights.pth'.format(args.model))